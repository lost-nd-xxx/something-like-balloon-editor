...
縦書きバルーン（SSP 2.8.80 以降の `vertical,1`）の設定・プレビューや、画像の回転・反転にも対応しています。
...
このアプリは設定や作業状態をすべてアプリのフォルダ内（`state.json` など）に保存します。アプリ自身がレジストリや AppData などフォルダの外に設定を書き込むことはありません。アプリのフォルダをまるごと削除するだけでアンインストールできます。

※ ファイルやフォルダを選ぶダイアログで「前回開いた場所」が記憶されているのは、Windows 標準のファイル選択画面が持つ機能によるものです（このアプリに限らず、多くのソフトで共通の動作です）。この記録は Windows 側に残るため、アプリのフォルダを削除しても残りますが、アプリの動作やデータには影響しません。
