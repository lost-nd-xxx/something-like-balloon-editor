...
`Cargo.toml` の依存（`[dependencies]` / `[*.dependencies]` / `[build-dependencies]` / `[dev-dependencies]`）を増減・変更したら、**必ず次の2ファイルも更新する**：

- `README.md` … 「使用ライブラリ」表に行を追加（クレート名・用途・ライセンス）。
- `ThirdPartyNotices.txt` … 当該クレートのライセンス全文を追記（手書き。生成ツールは未使用）。

この2ファイルは手動管理のため漏れやすい。**漏れ防止に pre-commit フックを用意している**（`.git/hooks/pre-commit`）。`Cargo.toml` の依存行を変更してコミットしようとすると、上記2ファイルがステージされていない場合はコミットが中断される。

- バージョン更新など依存追加に当たらない変更で止まった場合は `git commit --no-verify` でバイパス可。
- このフックは `.git/hooks/` 配下にあり **clone 先には自動で付かない**。別環境では同じ内容を `.git/hooks/pre-commit` に置き、実行権限を付与すること（`chmod +x`）。
